export const metadata = {
    title: 'MDX Test'
}
import { Footnote, FootnoteContent } from '@/components/Footnote'

# MDX Syntax Test Page
## Text Formatting
Regular text with **bold**, *italic*, and ***bold italic*** formatting.
This is `inline code` and ~~strikethrough text~~.

## Lists
### Unordered
- Item 1
- Item 2
  - Nested item 2.1
  - Nested item 2.2
    - Deep nested item

### Ordered
1. First item
2. Second item
   1. Nested numbered item
   2. Another nested item

## Code Blocks
### JavaScript
```javascript
const fibonacci = (n) => {
  if (n <= 1) return n;
  return fibonacci(n - 1) + fibonacci(n - 2);
};
```

### Python with line highlighting
```python {1,3-4}
def quick_sort(arr):
    if len(arr) <= 1:
        return arr
    pivot = arr[len(arr) // 2]
    left = [x for x in arr if x < pivot]
    middle = [x for x in arr if x == pivot]
    right = [x for x in arr if x > pivot]
    return quick_sort(left) + middle + quick_sort(right)
```

## Math Expressions
Inline math: $E = mc^2$

Display math:

$$\frac{12}{4}$$

## Tables
| Syntax | Description | Example |
|--------|-------------|---------|
| Header | Title | Here's this |
| Paragraph | Text | And more |
| Code | `code` | `const x = 1` |

## Blockquotes
> This is a blockquote
> > With a nested quote
> > > And another level

## Task Lists
<ul className="task-list">
  <li><span className="task-status">✓</span> <span className="completed">Write the tests</span></li>
  <li><span className="task-status">○</span> Run the tests</li>
  <li><span className="task-status">○</span> Fix the bugs</li>
</ul>

## Links and Images
[External Link](https://example.com)
![Alt text](https://via.placeholder.com/150)

## Footnotes
Here's a sentence with a footnote<Footnote index="A" />.

Here's another <Footnote />.

```tsx
import type { Metadata } from "next";
import { Geist, Geist_Mono } from "next/font/google";
import "./globals.css";
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faGithub, faDiscord, faXTwitter } from '@fortawesome/free-brands-svg-icons';
import { faEnvelope } from '@fortawesome/free-solid-svg-icons';
import Image from 'next/image';
import clouds from '@/app/clouds.png'

const geistSans = Geist({
  variable: "--font-geist-sans",
  subsets: ["latin"],
});

const geistMono = Geist_Mono({
  variable: "--font-geist-mono",
  subsets: ["latin"],
});

export async function generateMetadata({ params }) {
  console.log("yeag")
  return {
    title: {
      template: '%s - aurelium'
    },
  }
}

export default function RootLayout({
  children,
}: Readonly<{
  children: React.ReactNode;
}>) {
  return (
    <html lang="en">
      <body
        className={`${geistSans.variable} ${geistMono.variable} antialiased`}
      >
        <div className="container">
          <header>
            <div className="header-content">
              <div className="brand-container">
                <Image src={clouds} alt="clouds logo" width={24} height={24} id="clouds-header" />
                <div className="brand">aurelium</div>
              </div>
              <nav>
                <a href="/blog" className="nav-link">blog</a>
              </nav>
            </div>
          </header>

          <main className="main-content">
            {children}
          </main>


          <footer className="footer">
            <div className="social-links">
              <a href="https://github.com" target="_blank" rel="noopener noreferrer">
                <FontAwesomeIcon icon={faGithub} />
              </a>
              <a href="mailto:example@example.com">
                <FontAwesomeIcon icon={faEnvelope} />
              </a>
              <a href="https://discord.com" target="_blank" rel="noopener noreferrer">
                <FontAwesomeIcon icon={faDiscord} />
              </a>
              <a href="https://twitter.com" target="_blank" rel="noopener noreferrer">
                <FontAwesomeIcon icon={faXTwitter} />
              </a>
            </div>
            <div className="copyright">© aurelium 2024</div>
          </footer>
        </div>
      </body>
    </html>
  );
}
```

<FootnoteContent index="A">
Hello, world. These are the footnote contents.
</FootnoteContent>
<FootnoteContent>
Another footnote, this one automatic.
</FootnoteContent>